...
const SearchCars = ({ searchedCars, setSearchedCars }) => {
    ...
    const handleSearch = () => {
        if (selectedCompany === "All" || selectedCompany === "Company" || selectedCompany === "") {
    ...
    return (
        <div className="w-full bg-black p-6">
            <div className="flex flex-row rounded-lg max-w-[500px] bg-white m-auto p-1">
                <select
                    className="rounded-lg h-10 border-gray-400 border-2 p-1 w-48"
                ...
                <input
                    type="text"
                    placeholder="Model:"
                    className="rounded-lg border-2 border-gray-400 ml-2 pl-2"
                    value={model}
                    onChange={(e) => setModel(e.target.value)}
                />
                <button
                    className="rounded-md w-20 h-10 bg-black text-white ml-1"
